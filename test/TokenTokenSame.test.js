...
describe("TokenToken(Same Token) Test", () => {
  const accounts = [];
  let operator;
  let staker1;
  let staker2;
  ...
  it("Deploy Staking Pool(Not withdrawable)", async () => {
    ...
    notWithdrawablePool = await Pool.connect(operator).deploy(token.address, TOKEN_SYMBOL, token.address, TOKEN_SYMBOL);
